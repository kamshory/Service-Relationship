...
function parseISOMessage(targetElement)
{
    ...
    var attrTLV = closestElement.getAttribute('data-tlv') || '';
    var tlvField = attrTLV.split(',');
...
function parseTLVMessage(targetElement)
{
    var closestElement = targetElement.closest('.iso-container');
    var message = closestElement.querySelector('textarea.iso').value;
    var tlv = new TLV();
    var json = tlv.parse(message);
    closestElement.querySelector('.tlv-data-container').innerHTML = '';
    if(Object.size(json) > 0)
    {
        var tlvData = document.createElement('div');
        tlvData.classList.add('tlv-data');
        tlvData.innerHTML = renderTLVData(0, json);
        closestElement.querySelector('.tlv-data-container').appendChild(tlvData);
    }
}
...
function addTag(targetElement)
{
    var html = '<tr>\r\n'+
        '    <td><span class="field-remover"><a href="#" onclick="return removeElement(this)">&times;</a></span></td>\r\n'+
        '    <td><input type="text" class="field" value="" length="2" required="required" onchange="observeTag(this)"></td>\r\n'+
        '    <td><input type="text" class="value" value=""></td>\r\n'+
        '</tr>\r\n';
    var closestElement = targetElement.closest('.iso-container');
...
function observeTag(targetElement)
{
    var closestElement = targetElement.closest('.iso-container');
    var fields = closestElement.getElementsByClassName('field');
    var fieldList = [];
    for(var i = 0; i < fields.length; i++)
    {
        ...
        {
            console.log('Duplicated tag '+val);
            fields[i].value = '';
            fields[i].select();
            break;
        }
...
function buildTLV(targetElement)
{
    var tlv = new TLV();
    var closestElement = targetElement.closest('.iso-container');
    var fields = closestElement.getElementsByClassName('field');
    ...
    closestElement.querySelector('textarea.iso').value = tlv.build(json);
}